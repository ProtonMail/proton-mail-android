...
class MessagesListItemView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0)
	...
	fun bind(message: Message, labels: List<Label>, isMultiSelectionMode: Boolean, mailboxLocation: Constants.MessageLocationType, typeface: Typeface) {
		...
		val messageSenderText = when {
			Constants.MessageLocationType.fromInt(message.location) in arrayOf(Constants.MessageLocationType.ALL_DRAFT,
					Constants.MessageLocationType.ALL_SENT) -> message.toListStringGroupsAware
		...
		val isMessageSentAndTrashed=message.searchForLocation(Constants.MessageLocationType.ALL_SENT.messageLocationTypeValue)&&messageLabelsIDs.any {it==Constants.MessageLocationType.TRASH.messageLocationTypeValue.toString()}
		val isMessageSentAndArchived=message.searchForLocation(Constants.MessageLocationType.ALL_SENT.messageLocationTypeValue)&&messageLabelsIDs.any {it==Constants.MessageLocationType.ARCHIVE.messageLocationTypeValue.toString()}
		//region labels
		...
		messageLabelTrashTextView.visibility=if(mailboxLocation!=Constants.MessageLocationType.TRASH&&isMessageSentAndTrashed) View.VISIBLE else View.GONE
		messageLabelArchiveTextView.visibility=if(mailboxLocation!=Constants.MessageLocationType.ARCHIVE&&isMessageSentAndArchived) View.VISIBLE else View.GONE
		...
		if(mailboxLocation in arrayOf(Constants.MessageLocationType.STARRED,
			...
			val title=if(mailboxLocation==Constants.MessageLocationType.ALL_SENT)
...
//		return !messageId.isNullOrEmpty()&&PendingUpload.findByMessageId(messageId)!=null
	...
	private fun getLocationTitleId(mailboxLocation: Constants.MessageLocationType): Int? {
		return when (mailboxLocation) {
			...
			Constants.MessageLocationType.ALL_DRAFT -> R.string.drafts_option
			Constants.MessageLocationType.ALL_SENT -> R.string.sent_option
