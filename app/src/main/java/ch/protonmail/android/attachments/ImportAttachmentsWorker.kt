...
class ImportAttachmentsWorker(context: Context, params: WorkerParameters) : Worker(context, params) {
    ...
    override fun doWork(): Result {
        ...
        val fileUris = inputData.getStringArray(KEY_INPUT_DATA_FILE_URIS_STRING_ARRAY)?.mapNotNull { Uri.parse(it) }
        ...
        fileUris.filterNot {
            it.scheme == "file" && (it.path ?: "").contains(
                    ...
                    AppUtil.createTempFileFromInputStream(applicationContext, it)?.let { importedFile ->
                        ...
                        postImportAttachmentEvents.add(
                            PostImportAttachmentEvent(
                                Uri.fromFile(importedFile), displayName, size,
                                mimeType ?: Constants.MIME_TYPE_UNKNOWN_FILE, composerInstanceId
                            )
                        )
                    }
                }
            } catch (e: Exception) {
                Timber.i(e, "Import attachment failure")
            }
        }
