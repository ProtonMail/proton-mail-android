...
abstract class AttachmentMetadataDatabaseFactory : RoomDatabase() {
    ...
    companion object {
        private const val DEFAULT_DATABASE_FILENAME = "AttachmentMetadataDatabase.db"
        private val DATABASES = mutableMapOf<String, AttachmentMetadataDatabaseFactory>()

        @JvmOverloads
        @Synchronized
        fun getInstance(context: Context, username: String? = null): AttachmentMetadataDatabaseFactory {
        ...
        private fun buildDatabase(context: Context, username: String): AttachmentMetadataDatabaseFactory {
