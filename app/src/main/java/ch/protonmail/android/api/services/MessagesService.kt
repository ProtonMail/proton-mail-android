...
class MessagesService : JobIntentService() {
	...
	override fun onHandleWork(intent:Intent) {
		...
		when(intent.action) {
			ACTION_FETCH_MESSAGES_BY_PAGE-> {
				...
				if (Constants.MessageLocationType.fromInt(location) in listOf(Constants.MessageLocationType.LABEL, Constants.MessageLocationType.LABEL_FOLDER)) {
					val labelId = intent.getStringExtra(EXTRA_LABEL_ID)
					handleFetchFirstLabelPage(Constants.MessageLocationType.LABEL, labelId, currentUser)
				} else {
					handleFetchFirstPage(Constants.MessageLocationType.fromInt(location), refreshDetails, intent.getStringExtra(EXTRA_UUID), currentUser)
				}
			}
			ACTION_FETCH_MESSAGES_BY_TIME-> {
				...
				if(Constants.MessageLocationType.fromInt(location) in listOf(Constants.MessageLocationType.LABEL,
					...
					handleFetchMessagesByLabel(Constants.MessageLocationType.fromInt(location),labelTime,labelId, currentUser)
				} else {
	...
	private fun handleFetchFirstPage(location: Constants.MessageLocationType, refreshDetails: Boolean, uuid: String?, currentUser: String) {
		try {
			val messages = mApi.messages(location.messageLocationTypeValue, RetrofitTag(currentUser))
			handleResult(messages, location, refreshDetails, uuid, currentUser)
	...
	private fun handleFetchFirstLabelPage(location: Constants.MessageLocationType, labelId: String, currentUser: String) {
		try {
			val messagesResponse = mApi.searchByLabelAndPage(labelId, 0)
			handleResult(messagesResponse, location, labelId, currentUser)
		} catch (error: Exception) {
			AppUtil.postEventOnUi(MailboxLoadedEvent(Status.FAILED, null))
			Logger.doLogException(TAG_MESSAGES_SERVICE, "error while fetching messages", error)
		}
	...
	private fun handleFetchMessagesByLabel(location: Constants.MessageLocationType, unixTime: Long, labelId: String, currentUser: String) {
		try {
	...
	private fun handleResult(messages: MessagesResponse?, location: Constants.MessageLocationType, refreshDetails: Boolean, uuid: String?, currentUser: String) {
	...
	private fun handleResult(messagesResponse: MessagesResponse, location: Constants.MessageLocationType, labelId: String, currentUser: String) {
	...
	companion object {
		 ...
		 * Load initial page and detail of every message it fetch
		 */
		fun startFetchFirstPage(location: Constants.MessageLocationType, refreshDetails: Boolean, uuid: String?) {
			val context = ProtonMailApplication.getApplication()
			val intent = Intent(context, MessagesService::class.java)
			intent.action = ACTION_FETCH_MESSAGES_BY_PAGE
			intent.putExtra(EXTRA_MESSAGE_LOCATION, location.messageLocationTypeValue)
		 ...
		 */
		fun startFetchFirstPageByLabel(location: Constants.MessageLocationType, labelId: String?) {
			val context = ProtonMailApplication.getApplication()
			val intent = Intent(context, MessagesService::class.java)
			intent.action = ACTION_FETCH_MESSAGES_BY_PAGE
			intent.putExtra(EXTRA_MESSAGE_LOCATION, location.messageLocationTypeValue)
