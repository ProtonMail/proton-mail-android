...
class MessageFactory(private val attachmentFactory:IAttachmentFactory,
					 private val messageSenderFactory:IMessageSenderFactory):IMessageFactory {
			...
			message.location = it.LabelIDs!!
					...
					.fold(Constants.MessageLocationType.ALL_MAIL.messageLocationTypeValue) { location, newLocation ->
						if (newLocation !in listOf(Constants.MessageLocationType.STARRED.messageLocationTypeValue,
										...
										Constants.MessageLocationType.INVALID.messageLocationTypeValue) && newLocation < location) {
							newLocation
						} else
							location
					}
