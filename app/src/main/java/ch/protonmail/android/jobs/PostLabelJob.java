...
public class PostLabelJob extends ProtonMailEndlessJob {
    ...
    public void onRun() throws Throwable {
        ...
        if (!update) {
            labelResponse = getApi().createLabel(new LabelBody(labelName, color, display, exclusive));
        } else {
            labelResponse = getApi().updateLabel(labelId, new LabelBody(labelName, color, display, exclusive));
        }
